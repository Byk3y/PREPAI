-- ============================================================================
-- MIGRATION 029: Enhanced award_task_points with Server-Side Validation
-- ============================================================================
-- Purpose:
--   - Add server-side validation for each task's specific criteria
--   - Prevent users from gaming the points system
--   - Add negative points protection (clamp to >= 0)
--   - Validate streak increment (not reset) for maintain_streak task
--
-- Security Improvements:
--   - Previous version trusted client to only call when criteria were met
--   - This version validates criteria server-side before awarding points
--   - CRITICAL: Validates user authentication (auth.uid() IS NOT NULL)
--   - CRITICAL: Validates user ownership (p_user_id must match auth.uid())
--   - CRITICAL: Validates completion_date is not NULL
--   - P0: Fixed table name (audio_overviews, not audio_overview_jobs)
--   - P1: Rejects future completion dates
--   - P3: Handles race conditions (unique_violation) gracefully
--   - P4: Enhanced streak validation (checks updated_at to verify increment)
--   - P5: Basic audio playback validation (checks for completed audio_overviews)
--   - Improved timezone exception handling (catches specific exceptions)
--
-- Rollback Instructions:
--   Run migration 026_rpc_award_points.sql to restore previous version
-- ============================================================================

-- Drop existing function to replace it
DROP FUNCTION IF EXISTS award_task_points(UUID, TEXT, DATE);
...
BEGIN
  ...
  CASE p_task_key
    ...
    WHEN 'listen_audio_overview' THEN
      -- =====================================================================
      -- P5 FIX: Basic server-side validation for audio playback
      -- Validate: User has at least one completed audio overview
      -- Note: This doesn't track actual playback events, but verifies they have audio content
      -- Full validation would require playback event tracking table
      -- =====================================================================
      SELECT EXISTS (
    ...
    WHEN 'maintain_streak' THEN
      -- =====================================================================
      -- P4 FIX: Enhanced streak validation
      -- Check that user has a positive streak AND profile was updated today
      -- This is a heuristic: if profile.updated_at is today, streak was likely incremented
      -- Note: This doesn't perfectly verify increment vs reset, but is better than just checking > 0
      -- Full validation would require streak history table or trigger-based tracking
      -- =====================================================================
      SELECT 
        COALESCE(streak, 0) > 0 
      ...
      -- Explicitly handle missing profiles row (shouldn't happen, but be safe)
      -- If no row exists, v_criteria_met will be NULL, so set to false
...
-- =========================================================================
-- P3 FIX: Handle unique_violation specifically (race condition fix)
-- This handles concurrent completion attempts gracefully
-- =========================================================================
EXCEPTION 
...
-- ============================================================================
-- Test with invalid criteria (should fail):
-- SELECT award_task_points('user-uuid', 'study_15_minutes', CURRENT_DATE, 'UTC');
--
-- Test after meeting criteria:
-- INSERT INTO study_sessions (user_id, start_at, end_at, duration_seconds) 
--   VALUES ('user-uuid', NOW() - INTERVAL '20 minutes', NOW(), 1200);
-- SELECT award_task_points('user-uuid', 'study_15_minutes', CURRENT_DATE, 'UTC');
